...
public final class BodyInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (HttpManager.INSTANCE.mutualCallback != null) {
            ...
            if (((original.url().toString())).contains("verifySms")) {
                String mobile = "";
                for (int i = 0; i < 4; i++) {
                    if (((FormBody) original.body()).encodedName(i).equals("mobile")) {
                        mobile = ((FormBody) original.body()).encodedValue(i);
                        break;
                    }
                }
                oldMaps.put("verifySms", mobile);
            }
            bodys = HttpManager.INSTANCE.mutualCallback.getBodys(original.url().toString(), oldMaps);
        }

        if (bodys != null && bodys.size() > 0 && original.method().equals("GET") && !(requestBody instanceof MultipartBody)) {
