    ...
    private static boolean checkNetWork() {
        ...
        if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (wifiNetworkInfo != null && wifiNetworkInfo.isConnected()) {
                ...
                //获取移动数据连接的信息
                @SuppressLint("MissingPermission") NetworkInfo dataNetworkInfo = connMgr.getNetworkInfo(ConnectivityManager.TYPE_MOBILE);
            ...
            //获取所有网络连接的信息
            @SuppressLint("MissingPermission") Network[] networks = connMgr.getAllNetworks();
            if (networks != null && networks.length > 0) {
                for (int i = 0, len = networks.length; i < len; i++) {
                    @SuppressLint("MissingPermission") NetworkInfo networkInfo = connMgr.getNetworkInfo(networks[i]);
